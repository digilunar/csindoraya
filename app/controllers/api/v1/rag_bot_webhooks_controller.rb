...
class Api::V1::RagBotWebhooksController < ActionController::API
  def create
    ...
    if @rag_bot.use_general_ai_setting && @rag_bot.account.custom_ai_integration.present?
      ...
      # Search Python RAG Microservice for relevant context
      rag_context = ""
      begin
        rag_uri = URI.parse("http://172.17.0.1:8081/retrieve_context")
        rag_http = Net::HTTP.new(rag_uri.host, rag_uri.port)
        rag_http.read_timeout = 30
        
        rag_req = Net::HTTP::Post.new(rag_uri.request_uri, 'Content-Type' => 'application/json')
        rag_req.body = { query: payload['content'], top_k: 4 }.to_json
        
        rag_res = rag_http.request(rag_req)
        rag_parsed = JSON.parse(rag_res.body)
        
        if rag_parsed["status"] == "success" && rag_parsed["context"].present?
          rag_context = rag_parsed["context"]
        end
      rescue => e
        Rails.logger.error "Failed to retrieve context from RAG microservice: #{e.message}"
      end

      # Fallback to legacy knowledge if RAG is empty but legacy exists
      bot_knowledge_entries = @rag_bot.rag_knowledge_entries.map(&:answer).uniq.join("\n\n")
      if rag_context.present? || bot_knowledge_entries.present? || @rag_bot.rag_knowledge.present?
        system_prompt += "\n\nKonteks Pengetahuan:\n"
        system_prompt += "#{rag_context}\n" if rag_context.present?
        system_prompt += "#{@rag_bot.rag_knowledge}\n" if @rag_bot.rag_knowledge.present? && rag_context.blank?
        system_prompt += "\n#{bot_knowledge_entries}\n" if bot_knowledge_entries.present? && rag_context.blank?
      ...
      system_prompt += "2. Anda WAJIB memberikan jawaban akhir dalam format JSON murni dengan struktur: {\"status\": \"RELEVAN\" | \"TIDAK_RELEVAN\" | \"HANDOFF\", \"reply\": \"Jawaban Anda\"}.\n"
      system_prompt += "3. Jika pertanyaan melenceng, set status ke TIDAK_RELEVAN dan berikan pesan penolakan sopan di dalam reply. ANDA WAJIB MENGGUNAKAN BAHASA YANG SAMA DENGAN BAHASA YANG DIGUNAKAN OLEH USER.\n"
      system_prompt += "4. Jika relevan, set status ke RELEVAN dan tulis jawaban di dalam reply tanpa embel-embel.\n"
      system_prompt += "5. Jika pengguna secara eksplisit meminta berbicara dengan manusia, admin, CS, ATAU Anda mentok/tidak tahu jawabannya karena tidak ada di konteks, set status ke HANDOFF dan berikan pesan 'Mohon tunggu sebentar, saya akan mengalihkan percakapan ini ke tim terkait.' di dalam reply.\n"
      system_prompt += "6. Jika pesan user hanya berupa SAPAAN (seperti 'Halo', 'Hai', 'Malam', dsb), WAJIB balas dengan sapaan ramah di dalam reply (misal: 'Halo! Ada yang bisa kami bantu hari ini?').\n"
      system_prompt += "7. DILARANG KERAS menutup pesan dengan pertanyaan seperti 'Ada yang bisa saya bantu?' atau membuat daftar opsi menu layanan KECUALI merespons sapaan. Cukup jawab inti pertanyaannya saja.\n"
      system_prompt += "8. Gunakan format Markdown murni (* atau -) untuk list. JANGAN PERNAH menyebutkan bahwa Anda adalah AI atau Language Model.\n"
      ...
      if conversation_id
        ...
        if recent_messages.first && recent_messages.first.message_type != 'incoming'
      ...
      user_prompt += "Jika pertanyaan MELENCENG: {\"status\": \"TIDAK_RELEVAN\", \"reply\": \"\"}\n"
      user_prompt += "Jika mentok/harus ditransfer ke admin: {\"status\": \"HANDOFF\", \"reply\": \"Mohon tunggu sebentar...\"}\n"
      ...
      begin
        ...
        if parsed_response && parsed_response.dig('choices', 0, 'message', 'content')
          ...
          begin
            ...
            if json_reply['status'].to_s.upcase == 'HANDOFF'
              ai_reply = json_reply['reply'].presence || "Mohon tunggu sebentar, saya akan mengalihkan percakapan ini ke tim terkait."
              should_handoff = true
            elsif json_reply['status'].to_s.upcase == 'TIDAK_RELEVAN' && json_reply['reply'].blank?
    ...
    if ai_reply.present?
      ...
      if conversation_id
        ...
        if should_handoff
          team = @rag_bot.account.teams.find_by(name: 'Sekretariat')
          if team
            conversation.update!(team_id: team.id, status: :open)
            conversation.messages.create!(
              content: "Bot mengalihkan percakapan ini ke tim Sekretariat karena membutuhkan bantuan manusia.",
