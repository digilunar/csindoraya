...
module Rag
  class DocumentLoader
    ...
    def pdf_file?
      content_type == 'application/pdf' || file_path.end_with?('.pdf')
    end
    ...
    def image_file?
      content_type&.start_with?('image/') || file_path.match?(/\.(jpg|jpeg|png|gif)$/i)
    end
    ...
    def extract_from_excel
      ...
      rows = []
      spreadsheet.each { |row| rows << row.map(&:to_s).join(' | ') }
