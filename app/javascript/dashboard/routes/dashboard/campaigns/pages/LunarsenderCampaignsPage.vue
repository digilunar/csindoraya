...
import LunarsenderCampaignDialog from './LunarsenderCampaignDialog.vue';
...
const lunarsenderInboxes = computed(
  () => getters['inboxes/getLunarsenderInboxes'].value
);
...
<template>
  <CampaignLayout
    :header-title="t('CAMPAIGN.LUNARSENDER.HEADER_TITLE')"
    :button-label="t('CAMPAIGN.LUNARSENDER.NEW_CAMPAIGN')"
    @click="toggleWhatsAppCampaignDialog()"
    @close="toggleWhatsAppCampaignDialog(false)"
  >
    <template #action>
      <LunarsenderCampaignDialog
    ...
    <WhatsAppCampaignEmptyState
      v-else
      :title="t('CAMPAIGN.LUNARSENDER.EMPTY_STATE.TITLE')"
      :subtitle="t('CAMPAIGN.LUNARSENDER.EMPTY_STATE.SUBTITLE')"
      class="pt-14"
    />
